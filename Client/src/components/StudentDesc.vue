                    ...
                    <el-card shadow="hover" style="width:1500px">
                        <div v-if="info.sid != null">
                            ...
                            <div class="image-size">
                                <img :src="picURL + info.sfaceaddress" class="image-size-sub"/>
                            </div>
                            <!-- 学生的个人详细信息 -->
                        ...
                        <div v-if="info.uid != null">
                            ...
                            <!-- 学生头像 -->
                            <div class="image-size">
                                <img :src="picURL + info.ufaceaddress" class="image-size-sub"/>
                            </div>
                            <!-- 学生的个人详细信息 -->
    ...
    export default {
        ...
        data() {
            return {
                ...
                circleUrl:'https://ss0.bdstatic.com/70cFuHSh_Q1YnxGkpoWK1HF6hhy/it/u=1541043089,376401683&fm=26&gp=0.jpg', // 用户当前头像
                isSubmit:false,
        ...
        methods: {
            init(){ // 从服务器端获取当前页面要展示的内容并初始化
                ...
                var user = JSON.parse(sessionStorage.user).userid;
                ...
                if (user.userfaceaddress === ''){
                    user.userfaceaddress = this.circleUrl;
                }
            ...
            getCommentsByPage(){ // 获取当前用户查看对象的所有评论
                var id = sessionStorage.id.split(";");
                if (id[1] == "f" || id[1] == "F"){
                    this.halloffame = true;
                }
                ...
                result.then(response => {
                    if (response.data.status == 200){
                        if (response.data.allcomments.length == 0){
                            ...
                            this.commentvisible = false;
                                // response.data.allcomments[i].allSubComments[j]["approve"] = "el-icon-star-off"; // 控制是否修改已有评论的赞成数量
            ...
            tellToOthers(item,con){
                ...
                this.usercomment.comment = "";
                this.usercomment.comment = this.usercomment.comment + "<font size='5px' color = 'blue'>@" + con.usernickname + "</font>";
                this.$refs.uc.tellToOther(this.usercomment.comment);
