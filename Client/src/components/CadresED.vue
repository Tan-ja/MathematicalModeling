        ...
        <div style="width:1500px;heigth:100%;margin: 0 auto" v-show="!isDesc">
            <div>
                <el-breadcrumb separator-class="el-icon-arrow-right">
                    ...
                    <el-breadcrumb-item>干部风采</el-breadcrumb-item>
                </el-breadcrumb>
            </div>
            <el-divider></el-divider>
            <div  v-for="item in imagesTea" :key="item.sid" class="image-size" @click="saveDataStu(item.sid)">
                <el-image :src="picURL + item.sfaceaddress" class="image-size-sub"></el-image>      
                <div class="introce">
                    <div class="con-cla">{{item.sposition}}---{{item.sname}}</div>
                </div>
            </div>
            <div class="div-page">
                <el-pagination
    ...
    export default {
        ...
        methods:{
            getdata(){
                ...
                var teaResult = this.$axios.get("user/allstu?currentPage="+this.currentPage + "&pageSize=" + this.pageSize);
                teaResult.then((response) => {
                    if (response.data.status == 200){
                        this.picURL = this.$picURL;
                        if (response.data.students != null){
                            this.imagesTea = response.data.students.list;
                        }
                    }           
                    this.infoLength = response.data.students.total;
                });
                // } else {
                //     var teaResult = this.$axios.get("user/allstu?currentPage="+sessionStorage.currentPage + "&pageSize=" + sessionStorage.pageSize);
                //     teaResult.then((response) => {
                //         if (response.data.status == 200){
                //             if (response.data.students != null){
                //                 this.picURL = this.$picURL;
            ...
            saveDataStu(stusid){
                //将要显示详细信息的学生的id存入cookie中
                sessionStorage.id = "s:" + stusid;
                // this.$cookie.set("id", "s:" + stusid + "");
