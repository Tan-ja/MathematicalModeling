        ...
        <div style="width:1500px;heigth:100%;margin: 0 auto" v-show="!isDesc">
            <div>
                <el-breadcrumb separator-class="el-icon-arrow-right">
                    ...
                    <el-breadcrumb-item>数模风情</el-breadcrumb-item>
                    <el-breadcrumb-item>名师指导</el-breadcrumb-item>
                </el-breadcrumb>
            </div>
            <el-divider></el-divider>
            <div v-for="item in imagesTea" :key="item.uid" class="image-size" @click="savedata(item.uid)">
                <img :src="item.ufaceaddress" class="image-size-sub"/>    
                <div class="introce" v-show="!item.ushow">
                    <div class="con-cla">{{item.uname}}{{item.uprofession}}</div>
                </div>
            </div>
            <div class="div-page">
                <el-pagination
                @size-change="handleSizeChangeFor"
                @current-change="handleCurrentChangeFor"
                ...
                <el-col :span="28">
                    <el-card shadow="hover" style="width:1500px">
                        <div v-show="info.sid">
                            ...
                            <el-divider></el-divider>
                            <div class="image-size-desc">
                                <img :src="info.sfaceaddress" class="image-size-sub-desc"/>
                            </div>
                            <div v-html="info.sachieve">
                            </div>
                            <!-- <p class="ql-editor" v-html="info.sachieve"></p> -->
                        ...
                        <div v-show="info.uid">
                            ...
                            <div class="image-size-desc">
                                <img :src="info.ufaceaddress" class="image-size-sub-desc"/>
                            </div>
...
<script>
    ...
    export default {
        ...
        created(){
            ...
            // }
            // if (sessionStorage.pageSize != undefined && sessionStorage.pageSize !== ""){
        ...
        methods:{
            getdata(){
                var teaResult = this.$axios.get("user/alltea?currentPage=" + this.currentPage + "&pageSize=" + this.pageSize);
                teaResult.then((response) => {
                    if (response.data.status == 200){
                        if (response.data.teachers != null){
                            this.imagesTea = [];
                            for (let i = 0;i < response.data.teachers.list.length;i++){
                                let item = response.data.teachers.list[i];
                                item.ufaceaddress = this.$picURL + item.ufaceaddress;
                                this.imagesTea.push(item);
                            }
                            this.infoLength = response.data.teachers.total;
                        }
                    }       
                });
            },
            savedata(itemuid){
                //将要显示详细信息的指导老师的id存入sessionStorage中
                sessionStorage.id = "t:" + itemuid;
            ...
            handleSizeChangeFor(val) {
                this.pageSize = val;
                // sessionStorage.pageSize = val + "";
                this.getdata();
            },
            handleCurrentChangeFor(val) {
