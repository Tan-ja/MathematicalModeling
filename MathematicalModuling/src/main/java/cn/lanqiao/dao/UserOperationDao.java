...
public interface UserOperationDao {
	...
	void deleteUserOperationByUseridAndOperation(UserOperation userOperation);
	// 检查用户操作是否重复
	@Select("select * from useroperation where userid=#{userid} and otherid=#{otherid} and odate=#{odate} and operation=#{operation}")
