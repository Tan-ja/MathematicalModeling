<template>
    <div style="width:100%;height:100%">
        ...
        ref="uc"
        @change="updateNewData($event)"
        >
        </quill-editor>
        <div style="display:none">
            <el-upload
                :action="action"
                :before-upload="beforeUploadForVideo"
                :on-success="handleSuccessForVideo"
                :on-progress="uploadIng"
            >
            <el-button size="small" class="uploadvideo" type="primary">upload</el-button>
            </el-upload>
        </div>
        <div style="display:none">
            <el-upload
                :action="action"
                :before-upload="beforeUploadForImage"
                :on-success="handleSuccessForImage"
                :on-progress="uploadIng"
            >
            <el-button size="small" class="uploadimage" type="primary">upload</el-button>
            </el-upload>
        </div>
    </div>
</template>
<script>
    export default {
        ...
        methods:{
            ...
            handleSuccessForVideo(res, file, fileList){
                ...
                if (res !== ''){
                    quill.insertEmbed(length, 'video', this.$picURL + res, 'api');
                    this.content = this.content.replace(/<iframe/g,"<video");
                    this.content = this.content.replace(/iframe>/g,"video>");
            ...
            handleSuccessForImage(res, file, fileList){
                // 获取富文本组件实例
                let quill = this.$refs.uc.quill;
                let length = quill.getSelection().index; // 获取光标所在位置
                // insertEmbed(index: Number(插入的位置), type: String(标签类型), value: any(参数，将传入到create的方法中去), source: String = 'api')
                if (res !== ''){
                    quill.insertEmbed(length, 'image', this.$picURL + res, 'api');
                    quill.setSelection(length + 1); //光标位置向后移动一位
                    this.submit();
                } else {
                    this.$message.error(res.msg);    // 上传失败，提示错误信息
